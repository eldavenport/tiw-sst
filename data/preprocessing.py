...
def crop_region(theta_data: xr.DataArray, lat_min: float, lat_max: float, 
    ...
    if cropped_data.size == 0:
        raise ValueError(f"No data points found within bounds: "
...
def generate_training_sequences(input_anomalies: xr.DataArray, 
                              output_anomalies: xr.DataArray,
                              input_length: int, 
                              output_length: int, 
                              stride: int) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    """
    ...
    if total_time_steps < sequence_length:
        raise ValueError(f"Data too short: {total_time_steps} steps < {sequence_length} required")
...
def save_training_data_hdf5(input_sequences: np.ndarray, 
    ...
    with h5py.File(hdf5_path, 'w') as f:
        ...
        f.create_dataset('input_sequences', 
                        data=input_sequences.astype(np.float32),
                        compression='gzip', 
                        compression_opts=6,
                        chunks=True)
        
        f.create_dataset('output_sequences', 
        ...
        f.create_dataset('start_dates', 
                        data=start_dates_str.astype('S32'),  # 32-char strings
                        compression='gzip')
        f.create_dataset('end_dates', 
                        data=end_dates_str.astype('S32'),
                        compression='gzip')
...
def save_pytorch_training_data(input_sequences: np.ndarray, 
                             output_sequences: np.ndarray,
                             sequence_dates: pd.DataFrame,
                             input_region_bounds: Dict[str, float],
                             output_region_bounds: Dict[str, float],
...
def preprocess_tiw_data(raw_data_path: Path,
                       ...
                       stride_days: int = 3) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    results = preprocess_tiw_data(
        raw_data_path, processed_data_path, input_netcdf_filename,
